...
export const reportStore = defineStore("reportStore", () => {
  ...
  const comparisonFilesLookup: Ref<Map<string, Map<string, string>> | null> =
    ref(null);
  ...
  function loadReport(_files: File[], _submissionFiles: SubmissionFile[], fileName: string) {
    ...
    distribution.value = DistributionFactory.getDistributions(
      getFile("distribution.json").data
    );
    cliOptions.value = OptionsFactory.getCliOptions(
      getFile("options.json").data
    );
  ...
  function getComparisonFile(firstSubmissionId: string, secondSubmissionId: string) {
    ...
    const comparisonFile = comparisonFilesLookup.value.get(
      firstSubmissionId
  ...
  function getComparison(firstSubmissionId: string, secondSubmissionId: string) {
    const comparisonFileName = getComparisonFile(
      firstSubmissionId,
      secondSubmissionId
    );
    ...
    if (!filesOfFirstSubmission) {
      throw new Error(
        `Expected to find submission files for ${firstSubmissionId} but did not`
      );
    }
    if (!filesOfSecondSubmission) {
      throw new Error(
        `Expected to find submission files for ${secondSubmissionId} but did not`
      );
    }

    return ComparisonFactory.getComparison(comparisonFile.data, submissionFileIndex.data, filesOfFirstSubmission, filesOfSecondSubmission)
  }
  ...
  function getFile(fileName: string) {
    ...
    if (!file) {
      throw new Error(
        `Expected to find file ${fileName} in report but did not`
      );
    }
    return file;
  }
  ...
  return { loadReport, isReportLoaded, getFile, getComparison, getSubmissionCount, getReportFileName, includedComparisonCount, getCluster, getCliOptions, getDistributions, getRunInformation, getTopComparisons, getAllClusters, reset, getDisplayName, isAnonymized, getBaseCodeReport };
});
