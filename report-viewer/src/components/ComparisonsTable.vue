...
<table>
  ...
  <tr v-for="(comparison, index) in topComparisons"
    ...
    <td  @click="navigateToComparisonView(comparison.firstSubmissionId, comparison.secondSubmissionId)">{{ index + 1 }}.</td>
    ...
    <GDialog v-model="dialog">
      <ClustersList :clusters="getClustersFor(comparison.firstSubmissionId, comparison.secondSubmissionId)" :comparison="comparison"/>
    </GDialog>
  </tr>
</table>
</template>
