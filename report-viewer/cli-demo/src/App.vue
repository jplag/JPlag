<template>
  <div class="overflow-hidden">
    <div
      class="bg-background-light dark:bg-background-dark grid max-h-screen min-h-screen max-w-screen grid-rows-[1fr_1fr] overflow-hidden text-black dark:text-amber-50 print:max-h-none print:w-full print:max-w-full print:overflow-visible"
      :style="{
        'grid-template-columns': tableExpanded ? 'auto 0px 1fr' : 'auto 1fr auto'
      }"
    >
      <div
        class="bg-container-light dark:bg-container-dark border-container-border-light dark:border-container-border-dark col-span-3 col-start-1 row-start-1 min-h-16 border-b"
      >
        <TopBar />
      </div>
      ...
      <div class="col-start-2 row-start-2 p-5">
        <RouterView />
      </div>
