...
public final class TestDirectoryConstants {
	 ...
	 * path is extended with the "SUBMISSION_DIRECTORY_NAME", which is predefined in
	 * this class.
	 */
	public static final String TEMPORARY_SUBMISSION_DIRECTORY_NAME = Path.of("target", "testing-directory-submission")
			.toString();
	/**
	 * Create the complete path to the temporary result files. Here the temporary
	 * system path is extended with the "RESULT_DIRECTORY_NAME", which is predefined
	 * in this class.
	 */
	 ...
	 * resources.
	 */
	public static final Path BASE_PATH_TO_C_SHARP_RESOURCES_SORTALGO = Path.of("src", "test", "resources", "csharp",
			"sortAlgo");
	...
	public static final HashMap<LanguageOption, List<Path>> RESOURCE_PATH_MAPPER() {
		...
		languageSpecificResultMapper.put(LanguageOption.JAVA, Collections
				.unmodifiableList(Arrays.asList(BASE_PATH_TO_JAVA_RESOURCES_SORTALGO)));
		languageSpecificResultMapper.put(LanguageOption.C_SHARP, Collections
				.unmodifiableList(Arrays.asList(BASE_PATH_TO_C_SHARP_RESOURCES_SORTALGO)));
	...
	public static final HashMap<LanguageOption, Path> TEMPORARY_RESULT_PATH_MAPPER() {
		...
		languageSpecificResultMapper.put(LanguageOption.C_SHARP, Path.of(TEMPORARY_RESULT_DIRECTORY_NAME.toString() , LanguageOption.C_SHARP.toString()));
	...
	/**
	 * Base path to the saved results of the previous tests in a *.json file for
	 * java
	 */
	private static final Path BASE_PATH_TO_JAVA_RESULT_JSON = Path.of(BASE_PATH_TO_RESULT_JSON.toString(),
			"JavaResult.json");
	/**
	 * Base path to the saved results of the previous tests in a *.json file for
	 * csharp
	 */
	private static final Path BASE_PATH_TO_C_SHARP_RESULT_JSON = Path.of(BASE_PATH_TO_RESULT_JSON.toString(),
			"CSharpResult.json");
}
