...
Frequency Analysis
      --frequency-min-value=<frequencyStrategyMinValue>
                        max of min match length that will be compared and this  
                          value, is min size of considered submatches
      --frequency-strategy=<{COMPLETE_MATCHES, CONTAINED_MATCHES, SUB_MATCHES,
      WINDOW_OF_MATCHES}>
                        strategy for frequency Analysis, 
                        Options: 
                        completeMatches, containedMatches, subMatches, 
                        windowOfMatches
