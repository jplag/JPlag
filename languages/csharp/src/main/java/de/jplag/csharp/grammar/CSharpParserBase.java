...
public abstract class CSharpParserBase extends Parser {
    ...
    public void ReduceTree(ParserRuleContext currentctx) {
        ...
        if (!"yes".equalsIgnoreCase(reduceProperty))
            return;
    ...
    private static void reducer(ParserRuleContext node, boolean reduceAllChildren) {
        ...
        for (ParseTree child : node.children)
            if (child instanceof ParserRuleContext)
                reducer((ParserRuleContext) child, reduceAllChildren);
    ...
    public void ElementAccessSemanticCheck(ParserRuleContext currentctx) {
        ...
        if (lhsTarget.getRuleIndex() != ruleArrayCreationExpression && lhsTarget.getRuleIndex() != ruleStackallocExpression)
            return;
        ...
        if (lhsLastType != CSharpLexer.TK_RBRACK) {
            System.err.printf("%d:%d Error: Unexpected LHS last token %s (%d).%n", lhsLast.getLine(), lhsLast.getCharPositionInLine(),
    ...
    public void EnterNamespaceScope() {
        symTable.enterScope(CSharpScopeKind.Namespace);
    }

    public void EnterTypeScope() {
        symTable.enterScope(CSharpScopeKind.Type);
    }

    public void EnterBlockScope() {
        symTable.enterScope(CSharpScopeKind.Block);
    }
    ...
    public boolean IsValueTypeName() {
        ...
        if (type == CSharpLexer.KW_BOOL || type == CSharpLexer.KW_BYTE || type == CSharpLexer.KW_CHAR || type == CSharpLexer.KW_DECIMAL
                || type == CSharpLexer.KW_DOUBLE || type == CSharpLexer.KW_FLOAT || type == CSharpLexer.KW_INT || type == CSharpLexer.KW_LONG
                || type == CSharpLexer.KW_SBYTE || type == CSharpLexer.KW_SHORT || type == CSharpLexer.KW_UINT || type == CSharpLexer.KW_ULONG
                || type == CSharpLexer.KW_USHORT || type == CSharpLexer.TK_LPAREN)
            return true;
        CSharpScope.CSharpSymbol sym = symTable.currentScope().lookupChain(tok.getText());
        if (sym == null || sym.kind != CSharpSymbolKind.Type)
            return false;
    ...
    public boolean IsReferenceTypeName() {
        ...
        if (tok == null)
            return true;
        int type = tok.getType();
        if (type == CSharpLexer.KW_DYNAMIC || type == CSharpLexer.KW_OBJECT || type == CSharpLexer.KW_STRING || type == CSharpLexer.TK_LBRACK)
            return true;
        CSharpScope.CSharpSymbol sym = symTable.currentScope().lookupChain(tok.getText());
        if (sym == null)
            return true;
        if (sym.kind == CSharpSymbolKind.TypeParameter)
            return false;
        if (sym.kind == CSharpSymbolKind.Type && (sym.typeKind == CSharpTypeKind.Struct || sym.typeKind == CSharpTypeKind.Enum))
            return false;
        return true;
    }
    ...
    public boolean IsClassTypeName() {
        ...
        if (tok == null)
            return true;
        int type = tok.getType();
        if (type == CSharpLexer.KW_OBJECT || type == CSharpLexer.KW_STRING)
            return true;
        CSharpScope.CSharpSymbol sym = symTable.currentScope().lookupChain(tok.getText());
        if (sym == null)
            return true;
    ...
    public boolean IsClassBaseInterfaceList() {
        return classBaseTypeCheck(true);
    }

    public boolean IsClassBaseClassType() {
        return classBaseTypeCheck(false);
    }
    ...
    public boolean IsConstantPatternAhead() {
        ...
        if (t1 != null && t1.getType() == CSharpLexer.TK_LPAREN) {
            ...
            while (true) {
                ...
                if (tt == CSharpLexer.TK_LPAREN)
                    depth++;
                else if (tt == CSharpLexer.TK_RPAREN) {
                    depth--;
                    if (depth == 0)
                        break;
                } else if (tt == CSharpLexer.TK_COMMA && depth == 1)
                    return false;
            }
        }
        ...
        if (t1 != null && t1.getType() == CSharpLexer.Simple_Identifier) {
            ...
            try {
                par.type_();
                Token next = ((CommonTokenStream) _input).LT(1);
                if (next != null && next.getType() == CSharpLexer.TK_LPAREN)
                    return false;
            } catch (Exception e) {
            } finally {
                _input.seek(savedIndex);
    ...
    public boolean IsPositionalPatternAhead() {
        return !IsConstantPatternAhead();
    }
...
class CSharpScope {
    static class CSharpSymbol {
        ...
        CSharpSymbol(String name, CSharpSymbolKind kind) {
            this.name = name;
            this.kind = kind;
        }
    }
    ...
    void declare(CSharpSymbol s) {
        _symbols.put(s.name, s);
    }

    CSharpSymbol lookup(String n) {
        return _symbols.get(n);
    }
...
class CSharpSymbolTable {
    ...
    CSharpScope currentScope() {
        return _scopeStack.peek();
    }

    void enterScope(CSharpScopeKind kind) {
        _scopeStack.push(new CSharpScope(kind, _scopeStack.peek(), ""));
    }
    ...
    void declareType(String name, CSharpTypeKind kind, int arity) {
        ...
        if (arity > 0) {
            _genericArities.computeIfAbsent(name, k -> new HashSet<>()).add(arity);
        }
    }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if ("class".equals(txt) || "struct".equals(txt) || "interface".equals(txt) || "enum".equals(txt)) {
                ...
                if (j >= n)
                    continue;
                j = nextDefault(tokens, j + 1, n);
                if (j < n && "<".equals(tokens.get(j).getText()))
    ...
    private void registerType(String name, int arity, List<String> typeParams) {
        if (!isReserved(name))
            _knownTypeNames.add(name);
        if (arity > 0 && !isReserved(name))
            _genericArities.computeIfAbsent(name, k -> new HashSet<>()).add(arity);
        for (String tp : typeParams)
            if (isIdentLike(tp) && !isReserved(tp))
    ...
    private static int countTypeParams(TokenStream tokens, int pos, int n, List<String> names) {
        ...
        if (p >= n || !"<".equals(tokens.get(p).getText()))
            return 0;
    ...
    private static boolean isIdentLike(String t) {
        if (t == null || t.isEmpty())
            return false;
        char c = t.charAt(0);
    ...
    private static final Set<String> RESERVED = new HashSet<>(Arrays.asList("abstract", "as", "base", "bool", "break", "byte", "case", "catch",
            "char", "checked", "class", "const", "continue", "decimal", "default", "delegate", "do", "double", "else", "enum", "event", "explicit",
            "extern", "false", "finally", "fixed", "float", "for", "foreach", "goto", "if", "implicit", "in", "int", "interface", "internal", "is",
    ...
    private void populateBuiltins() {
        for (String t : new String[] {"bool", "byte", "sbyte", "char", "decimal", "double", "float", "int", "uint", "long", "ulong", "short",
                "ushort", "object", "string", "void", "dynamic", "Boolean", "Byte", "SByte", "Char", "Decimal", "Double", "Single", "Int16", "Int32",
                "Int64", "UInt16", "UInt32", "UInt64", "IntPtr", "UIntPtr", "Object", "String", "Void", "Guid", "DateTime", "DateTimeOffset",
                "TimeSpan", "Uri", "Exception", "Type", "Enum", "Delegate", "MulticastDelegate", "Attribute", "Math", "Convert", "Console",
                "Environment", "List", "IList", "IEnumerable", "IEnumerator", "ICollection", "IReadOnlyList", "IReadOnlyCollection",
                "IReadOnlyDictionary", "Dictionary", "IDictionary", "SortedDictionary", "SortedList", "HashSet", "SortedSet", "Queue", "Stack",
                "LinkedList", "LinkedListNode", "Func", "Action", "Predicate", "Comparison", "Converter", "Task", "ValueTask", "Nullable",
                "IAsyncEnumerable", "IAsyncEnumerator", "Span", "ReadOnlySpan", "Memory", "ReadOnlyMemory", "KeyValuePair", "Tuple", "ImmutableArray",
                "ImmutableList", "ImmutableDictionary", "ImmutableHashSet", "Lazy", "WeakReference", "EventHandler", "IEqualityComparer", "IComparer",
                "EqualityComparer", "Comparer", "ConcurrentDictionary", "ConcurrentQueue", "ConcurrentStack", "ConcurrentBag"})
