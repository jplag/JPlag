...
import static de.jplag.python.PythonTokenType.ELIF;
import static de.jplag.python.PythonTokenType.ELSE;
...
public class PythonLanguageTest extends LanguageModuleTest {
    ...
    protected void collectTestData(TestDataCollector collector) {
        collector.testFile("BasicTest.py").testSourceCoverage().testTokenSequence(IMPORT, IMPORT, CLASS_BEGIN, METHOD_BEGIN, ASSIGN, ASSIGN, LIST,
                ...
                ELIF, RETURN, LIST, IF_END, ASSIGN, LIST, WHILE_BEGIN, APPLY, APPLY, WHILE_END, RETURN, METHOD_END, METHOD_BEGIN, TRY_BEGIN, APPLY,
                RETURN, EXCEPT_BEGIN, RETURN, EXCEPT_END, TRY_END, METHOD_END, METHOD_BEGIN, ASSIGN, APPLY, ASSIGN, APPLY, ASSIGN, APPLY, ASSIGN,
                LIST, ASSIGN, APPLY, IF_BEGIN, APPLY, APPLY, IF_END, FOR_BEGIN, APPLY, IF_BEGIN, CONTINUE, IF_END, IF_BEGIN, BREAK, IF_END, APPLY,
                FOR_END, IF_BEGIN, NAMED_EXPR, APPLY, APPLY, IF_END, ASSIGN, LIST, IF_BEGIN, NAMED_EXPR, APPLY, APPLY, ELSE, APPLY, IF_END, TRY_BEGIN,
                ASSIGN, APPLY, APPLY, EXCEPT_BEGIN, APPLY, EXCEPT_END, TRY_END, ASSIGN, LAMBDA, LAMBDA, ASSIGN, LIST_COMPREHENSION, APPLY, RETURN,
                APPLY, METHOD_END, IF_BEGIN, ASSIGN, APPLY, APPLY, IF_END);
        ...
        collector.testFile("ControlFlowTest.py").testSourceCoverage().testTokenSequence(METHOD_BEGIN, ASSIGN, WHILE_BEGIN, ASSIGN, IF_BEGIN, BREAK,
                ...
                ELSE, PASS, WHILE_END, METHOD_END, METHOD_BEGIN, FOR_BEGIN, APPLY, IF_BEGIN, BREAK, IF_END, ELSE, PASS, FOR_END, METHOD_END, IF_BEGIN,
                APPLY, APPLY, APPLY, APPLY, APPLY, IF_END);
        ...
        collector.testFile("ExceptionGroupTest.py").testSourceCoverage().testTokenSequence(METHOD_BEGIN, METHOD_BEGIN, ASSIGN, LIST, APPLY, APPLY,
                APPLY, APPLY, IF_BEGIN, RAISE, APPLY, IF_END, METHOD_END, TRY_BEGIN, APPLY, EXCEPT_BEGIN, APPLY, EXCEPT_END, EXCEPT_BEGIN, APPLY,
                EXCEPT_END, EXCEPT_BEGIN, APPLY, EXCEPT_END, TRY_END, METHOD_END, METHOD_BEGIN, METHOD_BEGIN, ASSIGN, LIST, APPLY, APPLY, ASSIGN,
                LIST, APPLY, TRY_BEGIN, RAISE, APPLY, EXCEPT_BEGIN, APPLY, RAISE, APPLY, EXCEPT_END, TRY_END, METHOD_END, TRY_BEGIN, APPLY,
                EXCEPT_BEGIN, APPLY, EXCEPT_END, EXCEPT_BEGIN, APPLY, EXCEPT_END, EXCEPT_BEGIN, APPLY, EXCEPT_END, EXCEPT_BEGIN, APPLY, EXCEPT_END,
                TRY_END, METHOD_END, IF_BEGIN, APPLY, APPLY, IF_END);
