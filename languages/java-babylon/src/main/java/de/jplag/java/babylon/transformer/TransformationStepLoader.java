...
/**
 * Utility class for loading installed {@link TransformationPipeline.Step} implementations.
...
public class TransformationStepLoader {
    ...
    /**
     * Get all transformation steps that are currently in the classpath.
     * The result will be cached.
     * Use {@link #clearCache()} to obtain new instances.
     *
     * @return the transformation steps as an unmodifiable map from identifiers to {@link TransformationPipeline.Step} instances
     */
        ...
        if (cachedStepInstances == null) {
            synchronized (TransformationStepLoader.class) {
                if (cachedStepInstances == null) {
                    ...
                    if (stepLoader == null) stepLoader = ServiceLoader.load(TransformationPipeline.Step.class);
                    if (simpleTransformationLoader == null) simpleTransformationLoader = ServiceLoader.load(SimpleTransformation.class);

                    List<TransformationPipeline.Step> stepsList = Stream.concat(
                            stepLoader.stream().map(ServiceLoader.Provider::get),
                            simpleTransformationLoader.stream().map(ServiceLoader.Provider::get).map(SimpleTransformationStep::new)
                    ).toList();
                    ...
                    for (TransformationPipeline.Step step : stepsList) {
                        ...
                        if (steps.remove(identifier) != null && skipped.add(identifier)) {
                            logger.error("Multiple implementations for a transformation step '{}' are present in the classpath! Skipping ..", identifier);
                            continue;
                        }
    ...
    public static void clearCache() {
        synchronized (TransformationStepLoader.class) {
            ...
            if (stepLoader != null) stepLoader.reload();
            if (simpleTransformationLoader != null) simpleTransformationLoader.reload();
        }
    }
}
