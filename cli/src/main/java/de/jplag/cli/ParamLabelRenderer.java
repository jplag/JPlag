...
public class ParamLabelRenderer implements CommandLine.Help.IParamLabelRenderer {
    ...
    @Override
    public CommandLine.Help.Ansi.Text renderParameterLabel(CommandLine.Model.ArgSpec argSpec, CommandLine.Help.Ansi ansi, List<CommandLine.Help.Ansi.IStyle> styles) {
        if(argSpec.type().isEnum()) {
            @SuppressWarnings("unchecked") Enum<?>[] enumConstants = ((Class<Enum<?>>)argSpec.type()).getEnumConstants();
