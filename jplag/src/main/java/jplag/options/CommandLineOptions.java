  ...
  public void initializeSecondStep(Program program) throws jplag.ExitException {
      ...
      for (int j = 0; j < languages.length - 1; j += 2) {
          if (languageName.equals(languages[j])) {
              try {
                  Constructor<?>[] languageConstructors = Class.forName(languages[j + 1])
                  ...
                  e.printStackTrace();
                  throw new jplag.ExitException("Illegal value: Language instantiation failed",
                      ExitException.BAD_LANGUAGE_ERROR);
              }
          }
      }
    if (!found1) {
      throw new jplag.ExitException(
          "Illegal value: Language instantiation failed: Unknown language \"" + languageName + "\"",
          ExitException.BAD_LANGUAGE_ERROR);
    }
