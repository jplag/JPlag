/*
 * Diese Klasse implementiert den GSTiling Algorithmus Allerdings ist sie sehr spezifisch auf die Klassen "Structure",
 * "Token", sowie "Matches" und "Match" ausgelegt
 */
public class GSTiling implements TokenConstants {
    private Matches matches = new Matches();

    /*
     * creating hashes. The hash-code will be written in every Token for the next <hash_length> token. (includes the Token
     * itself) Das Ganze laeuft in linearer Zeit. condition: 1 < hashLength < 26 !!!
     */
    ...
    public final void compare(Submission subA, Submission subB) {
        Submission A, B, tmp;
    ...
    private final void compare(Submission subA, Submission subB, int mml) {
    ...
    public final void compareWithBasecode(Submission subA, Submission subB) {
        Submission A, B, tmp;
        if (subA.struct.size() > subB.struct.size()) {
            A = subB;
            B = subA;
    ...
    private final void compareWithBasecode(Submission subA, Submission subB, int mml) {
        Structure structA = subA.struct;
        Structure structB = subB.struct;
