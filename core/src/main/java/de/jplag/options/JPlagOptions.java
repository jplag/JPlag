 ...
 */
@RecordBuilder()
public record JPlagOptions(Language language, Integer minimumTokenMatch, Set<SubmissionDirectory> submissionDirectories,
                           Set<SubmissionDirectory> oldSubmissionDirectories,
                           SubmissionIdentifier baseCodeSubmission, String subdirectoryName, List<String> fileSuffixes,
                           String exclusionFileName,
                           SimilarityMetric similarityMetric, double similarityThreshold,
                           int maximumNumberOfComparisons, ClusteringOptions clusteringOptions,
                           boolean debugParser, MergingOptions mergingOptions, boolean normalize,
                           boolean analyzeComments,
     ...
     */
    public JPlagOptions(Language language, Set<SubmissionDirectory> submissionDirectories, Set<SubmissionDirectory> oldSubmissionDirectories) {
     ...
     */
    public JPlagOptions(Language language, Integer minimumTokenMatch, Set<SubmissionDirectory> submissionDirectories, Set<SubmissionDirectory> oldSubmissionDirectories,
                        SubmissionIdentifier baseCodeSubmission, String subdirectoryName, List<String> fileSuffixes, String exclusionFileName,
    ...
    @Deprecated
    public JPlagOptions withBaseCodeSubmissionDirectory(File baseCodeDirectory) {
        Set<SubmissionDirectory> allDirectories = new HashSet<>();
        ...
        if(matchingDirectory.isPresent()) {
            return this.withBaseCodeSubmission(new SubmissionIdentifier(matchingDirectory.get().name(), baseCodeDirectory.getName()));
        } else {
            FileSystemSingleSubmissionDirectory baseCodeSubmissionDirectory = new FileSystemSingleSubmissionDirectory(baseCodeDirectory, "baseCode"); //TODO name?
            Set<SubmissionDirectory> copy = new HashSet<>(oldSubmissionDirectories);
            copy.add(baseCodeSubmissionDirectory);
            return withOldSubmissionDirectories(copy).withBaseCodeSubmission(baseCodeSubmissionDirectory.getSubmissionIdentifer());
        }
    }

    @Deprecated
    public JPlagOptions withBaseCodeSubmissionName(String name) {
        return this.withBaseCodeSubmission(new SubmissionIdentifier(null, name));
    }
}
